 ...
 * 100% относительно родителя), плюс тексту напрямую задаётся фиксированная ширина
 * (text.width, а не resize) — рабочий паттерн, который переносит текст по ширине
 * независимо от того, сработает ли horizontalSizing:'fill' у обёртки.
 *
...
function insertSticker(plan) {
  ...
  const titleWrapper = createTextWrapper(
    plan.title, plan.fontFamily, plan.titleFontSize, "700", plan.textColor,
    plan.width - plan.padding * 2
  );
  board.appendChild(titleWrapper);

  if (plan.text) {
    const bodyWrapper = createTextWrapper(
